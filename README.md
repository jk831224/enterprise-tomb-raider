...
│       ├── annual-report-analysis.md  ← v1.3 新增
│       ├── company-deep-dive.md
│       └── decision-brief.md
├── references/            # 共用知識層（Skill 和 Agent 都引用）
│   ├── methodology/       # 路徑選擇、規模分類、品質檢查、年報來源、drop zone
│   ├── templates/         # 報告結構模板
│   └── cases/             # 實戰案例與已知陷阱
├── input/                 # ★ Drop Zone：使用者預先餵入的年報、截圖、筆記（v1.4）
├── product/               # ★ PM 工件層（v1.4）
│   ├── PRD.md             # 主 PRD
│   ├── architecture.md    # 系統架構文件
│   ├── flow-diagram.md    # 系統流程圖（Mermaid）
│   ├── design-rationale.md # 設計意圖與方法論解釋
│   ├── rfcs/              # 設計變更紀錄（每個迭代一份 RFC）
│   └── perf/              # 效能追蹤報告與 baseline
└── output/                # 報告產出（.gitignore 排除）
```
