...
| | 本專案 | 一般 Prompt 集合 | Python/JS Agent 框架 | ChatGPT GPTs |
|--|--------|-----------------|---------------------|--------------|
| 實作方式 | 純 Markdown，零程式碼 | 純文字 | 需要寫程式 | GUI 設定 |
| 執行環境 | Claude Code CLI | 複製貼上 | 獨立運行 | ChatGPT 網頁 |
| 多階段流程 | Skill → Agent → 品質 Review 三層分離 | 沒有流程概念 | 程式碼定義流程 | 單輪或簡單多輪 |
| Context 管理 | 按階段動態載入 prompt，控制 token 用量 | 全部塞進去 | 程式碼控制 | 平台管理 |
| 知識累積 | 案例沉澱機制，Agent 從歷史學習 | 無 | 需自建 | 無 |
| 品質控制 | Rules 自動生效 + 四級證據標記 | 靠使用者自己判斷 | 需自建 | 無 |
