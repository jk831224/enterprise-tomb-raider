# Recon — 系統架構文件

> 最後更新：2026-04-06

## 系統定位

Recon 是一套基於 Claude 的產業與公司情報研究系統，將「對話引導」與「自主研究」拆成兩個獨立的執行層，透過結構化參數傳遞串接。

**一句話架構**：使用者透過 `/recon` 指令啟動 → Skill 層釐清需求並產出結構化參數 → Agent 層自主執行多階段研究 → 結構化報告產出後品質 review。

**入口**：透過 `.claude/skills/` 定義的 `/recon`、`/industry`、`/company` 指令觸發。

---

## 架構總覽

```
┌─────────────────────────────────────────────────────┐
│                      使 用 者                        │
│         /recon  ·  /industry  ·  /company            │
└────────────────────────┬────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────┐
│              .claude/skills/ (入口層)                 │
│                                                     │
│  ・需求界定（Scoping）                                │
│  ・路徑選擇（A: 產業→公司 / B: 公司→產業）            │
│  ・流程控制（按階段載入 prompt、階段轉換）              │
│  ・品質 Review                                       │
└────────────────────────┬────────────────────────────┘
                         │ 載入 AGENT.md + 階段 prompt
                         ▼
┌─────────────────────────────────────────────────────┐
│              agent/ (執行層)                          │
│                                                     │
│  ・自主搜尋迴圈（web_search + web_fetch）              │
│  ・交叉驗證                                          │
│  ・降級策略                                          │
│  ・結構化產出                                         │
└────────────────────────┬────────────────────────────┘
                         │ 引用
                         ▼
┌─────────────────────────────────────────────────────┐
│              references/ (知識層)                     │
│                                                     │
│  ・methodology/  方法論                               │
│  ・templates/    報告模板                              │
│  ・cases/        實戰案例                              │
└────────────────────────┬────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────┐
│              output/ + .claude/rules/                 │
│              報告產出（自動受品質規則約束）               │
└─────────────────────────────────────────────────────┘
```

---

## 職責切割

### 入口層（Skills）

**檔案**：`.claude/skills/{recon,industry,company}/SKILL.md`

| 職責 | 說明 |
|------|------|
| 需求界定 | 判斷路徑 A（產業→公司）或路徑 B（公司→產業） |
| Scoping | 引導使用者釐清細分領域、地理範圍、分析目的 |
| 規模預判 | 依標準將目標分類為微型/中型/大型，決定分析策略 |
| 流程控制 | 按階段載入 AGENT.md 和對應 prompt，驅動研究執行 |
| 階段轉換 | 各階段產出後，引導使用者確認並決定下一步 |
| 品質 Review | 用 quality-checklist.md 檢視最終報告 |

### 執行層（Agent）

**檔案**：`agent/AGENT.md` + `agent/prompts/*.md`（動態載入）

| 職責 | 說明 |
|------|------|
| 接收參數 | 從 Skill 層接收結構化輸入 |
| 多階段研究 | 依序執行：實體驗證 → 利害關係人調查 → 產業分析 → 公司深度分析 |
| 自主搜尋迴圈 | 每階段內部執行「搜尋→評估→補充搜尋」迴圈直到滿足產出標準 |
| 降級處理 | 搜尋上限內無法滿足標準時，標註證據等級後降級產出 |
| 結構化產出 | 依 templates/ 格式產出報告 |

**關鍵機制：Prompt 動態載入**
Agent 不一次載入所有階段 prompt。進入每個階段時才載入對應的 `prompts/*.md`，減少 context window 消耗。

```
agent/prompts/
├── entity-verification.md        # 法律實體驗證（路徑 B Step 1）
├── stakeholder-investigation.md  # 利害關係人調查（路徑 B Step 1.5）
├── industry-analysis.md          # 產業深度分析（路徑 A 主體 / 路徑 B Step 2）
└── company-deep-dive.md          # 公司深度分析（路徑 B Step 3）
```

### 知識層（References）

**被引用者**：入口層 + 執行層
**設計目的**：避免知識雙寫與同步問題

```
references/
├── methodology/
│   ├── path-selection.md          # 路徑判斷規則（Skill 引用）
│   ├── scale-classification.md    # 規模分類標準（Skill + Agent 引用）
│   └── quality-checklist.md       # 品質檢查清單（Skill Review 時引用）
├── templates/
│   ├── industry-report.md         # 產業報告模板（Agent 產出時引用）
│   └── company-report.md          # 公司報告模板（Agent 產出時引用）
└── cases/
    ├── _case-template.md          # Case 標準結構
    └── vmfive-2026.md             # 實戰案例：VMFive 調查
```

---

## 資料流：兩條分析路徑

### 路徑 A：產業 → 公司

```
使用者：「幫我研究 XX 產業」
         │
    /recon: 路徑判斷 → A
         │
    /recon: Scoping（細分領域 + 地理範圍 + 分析目的）
         │
    /recon: 交接參數 ──────────────────────────────┐
                                                   ▼
                                    Agent: 載入 industry-analysis.md
                                              │
                                    Agent: 搜尋迴圈（≤20 search + 10 fetch）
                                              │
                                    Agent: 產出產業報告
                                              │
    /recon: 呈現產業報告 ◀─────────────────────────┘
         │
    /recon: 「要深入某家公司嗎？」
         │
         ├─ 否 → 流程結束
         └─ 是 → 進入路徑 B（從 Step 1 開始）
```

### 路徑 B：公司 → 產業

```
使用者：「幫我研究 XX 公司」
         │
    /recon: 路徑判斷 → B
         │
    /recon: Scoping（公司名 + 分析目的 + 關注重點）
         │
    /recon: 規模預判
         │
    /recon: 交接參數 ──────────────────────────────┐
                                                   ▼
                              ┌─── Agent: Step 1 ─────────────────┐
                              │    載入 entity-verification.md     │
                              │    法律實體驗證 + 規模分類確認      │
                              └─────────────┬─────────────────────┘
                                            │
    /recon: 確認基本輪廓 ◀───────────────────┘
         │
         ▼
                              ┌─── Agent: Step 1.5 ───────────────┐
                              │    載入 stakeholder-investigation.md│
                              │    三層深度調查                     │
                              │    （基本盤→關係網絡→影響鏈）       │
                              └─────────────┬─────────────────────┘
                                            │
    /recon: 確認調查結果 ◀───────────────────┘
         │
         ▼
                              ┌─── Agent: Step 2 ─────────────────┐
                              │    載入 industry-analysis.md        │
                              │    以目標公司為錨點的產業分析        │
                              └─────────────┬─────────────────────┘
                                            │
                              ┌─── Agent: Step 3 ─────────────────┐
                              │    載入 company-deep-dive.md        │
                              │    整合前三階段的公司深度分析        │
                              └─────────────┬─────────────────────┘
                                            │
    /recon: 品質 Review ◀────────────────────┘
         │
    /recon: 引導使用者用 quality-checklist 檢視
         │
         ▼
    output/{date}_{company}_{type}.md
```

---

## 搜尋迴圈與終止機制

Agent 每個研究階段內部都運行同一套迴圈邏輯：

```
ENTER 階段
  │
  ├─ 載入對應 prompt
  │
  ▼
  WHILE 產出未達完成標準:
  │
  ├─ 執行搜尋（web_search / web_fetch）
  ├─ 評估已蒐集資訊是否足夠
  │
  ├─ IF 足夠 → 產出結構化結果 → EXIT 迴圈
  ├─ IF 不足 → 識別缺口 → 調整搜尋策略 → 繼續
  └─ IF 達搜尋上限 → 降級產出 → EXIT 迴圈
```

**搜尋上限（防止無限展開）**：

| 階段 | web_search | web_fetch | 附加限制 |
|------|------------|-----------|---------|
| 每個研究階段 | 20 次 | 10 次 | — |
| 利害關係人調查（每位人物） | 5 次 | — | — |
| 關聯實體追蹤 | — | — | 最多展開 2 層 |

**四級證據標記**：

| 等級 | 條件 | 標記方式 |
|------|------|---------|
| 充分證據 | 多來源交叉驗證 | 正常陳述 |
| 部分證據 | 單一來源或間接推論 | `[部分證據]` |
| 證據不足 | 搜尋未果但有合理推測 | `[推測，待驗證]` |
| 無資料 | 完全搜尋不到 | `[資料缺失]` + 已嘗試策略 |

---

## 規模自適應策略

規模分類直接影響 Agent 的分析框架選擇：

| 維度 | 微型（<50人） | 中型（50-500人） | 大型/上市 |
|------|-------------|-----------------|----------|
| 財務分析 | 現金流結構推論 | 融資輪次分析 | 完整三大報表 |
| 組織分析 | 跳過組織圖 | 標準框架 | 完整組織分析 |
| 利害關係人 | 關鍵人物 + 關係資本（核心） | 標準三層調查 | 完整三層 + 董事會分析 |
| 產業分析 | 簡化版五力 | 標準五力 | 完整五力 + 數據支撐 |
| 盡職調查 | 法律實體 + 關鍵人物風險 | 標準框架 | 完整 DD 框架 |

---

## 案例學習機制

每次完成分析後，沉澱一個 case 到 `references/cases/`。

```
case 結構：
├── 分析摘要（目標、路徑、結果）
├── 踩過的坑（現象 → 根因 → 修正 → 教訓）
├── 有效搜尋策略
├── 無效搜尋策略
├── 規模適配反思
└── 品質自評
```

Agent 在執行前會讀取相關 case，避免重蹈覆轍。目前已有案例：
- `vmfive-2026.md`：AdTech 微型企業調查，記錄了 4 個常見調查陷阱

---

## `.claude/` 配置層

`.claude/` 資料夾為 Claude Code 提供專案級配置，解決了原始設計中「Claude Code 裡 Skill 層角色模糊」的問題。

### 結構

```
.claude/
├── settings.json          # 權限配置（自動允許研究工具）
├── skills/                # 自定義指令
│   ├── recon/SKILL.md     # /recon — 統一入口，完整流程
│   ├── industry/SKILL.md  # /industry — 路徑 A 快捷入口
│   └── company/SKILL.md   # /company — 路徑 B 快捷入口
└── rules/                 # 自動載入的品質規則
    └── output-quality.md  # 報告品質標準（檔名、必要元素、語言規範）
```

### 設計決策

| 決策 | 選擇 | 原因 |
|------|------|------|
| 三個 skill 而非一個 | `/recon`（完整）、`/industry`（路徑A）、`/company`（路徑B） | 讓進階使用者跳過路徑判斷，直接啟動 |
| Rules 只管輸出品質 | 研究執行規則留在 `AGENT.md` | 避免雙來源，AGENT.md 是執行規格的唯一權威 |
| 權限預設允許 | WebSearch、WebFetch 等研究工具自動允許 | 研究流程需要大量搜尋，每次都問權限會中斷節奏 |

---

## 已知限制與待改善項目

### 已知限制

1. **Skill/Agent 交接依賴人工**：目前沒有自動化的參數格式驗證。Skill 產出的參數包如果缺少必要欄位，Agent 不會主動報錯，而是用預設值填補。
2. **Output 無索引**：報告累積後缺乏搜尋與管理機制。
3. **Case 基數小**：僅 1 個案例，學習效果有限。
4. **搜尋工具依賴**：研究品質高度依賴 web_search 和 web_fetch 的可用性與回傳品質。

### 可能的改善方向

- 加入 `output/INDEX.md` 自動索引機制
- 為 Skill → Agent 的參數交接定義 schema 驗證
- 累積更多 case 以強化 Agent 的調查直覺
- 考慮 `Stop` hook 在研究完成時自動觸發品質 checklist
