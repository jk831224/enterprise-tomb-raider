...
**一句話架構**：使用者透過 `/recon` 指令啟動 → Skill 層釐清需求 + 預分析評估 → Agent 層自主執行多階段研究（含年報解析）→ 結構化報告產出後品質 review。

**入口**：透過 `.claude/skills/` 定義的 `/recon`、`/industry`、`/company` 指令觸發。

> 完整流程圖見 [`docs/flow-diagram.md`](flow-diagram.md)（Mermaid 格式，可視化渲染）。
...
> 詳細流程圖（含檔案調用關係）見 [`docs/flow-diagram.md`](flow-diagram.md)。
...
使用者：「幫我研究 XX 公司」
    ...
    references/cases/case-{company}.md
```
