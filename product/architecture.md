...
│                                                     │
│  ・_registry.json   CRM 式結構化企業 DB（v1.6）         │
│  ・_index.md        人類可讀索引（自動生成）              │
└─────────────────────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────┐
│              scripts/ (工具層)                         │
│                                                     │
│  ・sync-registry.py  掃描 reports → 更新 registry     │
│                      → 生成 index → 篩選查詢（v1.6）   │
│                                                     │
│  ・mcp/tw-data-server.py  Playwright MCP server      │
│    → tw_company_lookup   台灣公司登記（findbiz+twincn）│
│    → tw_person_network   關聯法人搜尋（twincn）        │
│    → headless_fetch      通用 JS 渲染頁面抓取          │
│    （v1.8 planned，RFC-004）                          │
└─────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────┐
│              .claude/hooks/ (防線層，v1.9)             │
│                                                     │
│  ・enforce-stage-prompt-load.sh                      │
│    → PreToolUse on Write|Edit                       │
│    → 若目標為 cases/**/*_{stage}.md 且 session 未    │
│      Read 對應 agent/prompts/{stage}.md，exit 2 擋下 │
│    → 擋 recon 六階段產物：entity / stakeholder /     │
│      industry / company / decision-brief / supplement│
│    （v1.9 新增，RFC-006）                            │
└─────────────────────────────────────────────────────┘
```
...
**關鍵機制：Stage Discipline Hook（v1.9）**
Prompt 動態載入在壓力下會被模型自主豁免（「我記得怎麼寫」）。PreToolUse hook 在 Write/Edit 目標為 `cases/**/*_{stage}.md` 時掃 transcript，沒 Read 對應 prompt 就 block。這是防線層（見上圖 `.claude/hooks/`），確保「動態載入」從慣例變成剛性規則。詳見 [RFC-006](rfcs/RFC-006-stage-discipline-hook.md)。
...
## Cases Registry（v1.6 新增）

### 定位

解決 case 數量增長後的跨案例管理需求。不在 Agent 執行路徑上——Agent 不需要讀 registry 來完成分析；registry 是分析完成後的管理層工具。

### 架構

```
cases/
├── _registry.json    ← 結構化 DB（source of truth）
├── _index.md         ← 人類可讀索引（自動生成）
├── _case-template.md
├── README.md
└── {公司名稱}/       ← 個別 case（由 Agent 產出）

scripts/
└── sync-registry.py  ← 掃描 cases/ → 更新 registry → 生成 index
```

### 資料流

```
Agent 產出報告 → cases/{target}/company-report.md
                         │
                   python sync-registry.py sync
                         │
                         ▼
               解析報告 metadata（3 種格式）
               解析公司基本資訊 table
...
| 欄位群 | 欄位 | 來源 |
|--------|------|------|
| CRM 基本 | name_zh, name_en, aliases, tax_ids | auto-scan |
| 產業分類 | sector (8 大類), industry (自由文字), tags | 手動 enrichment |
| 企業規模 | size, listing, employee_range, capital_ntd | auto-scan |
| 分析狀態 | first_analyzed, last_updated, report_version, route | auto-scan |
| 評估摘要 | risk_level, verdict, red_flags, opportunities | 手動 enrichment |
| 財務快照 | revenue_estimate, revenue_confidence, funding_stage | 手動 enrichment |

### Merge 策略

- **Always update**：last_updated, report_version, report_path, brief_path
- **Additive**：tax_ids（union）
- **Only fill if empty**：所有其他欄位（手動 enrichment 永遠優先）
...
2. ~~**Output 無索引**：報告累積後缺乏搜尋與管理機制。~~ → v1.5 `cases/` 統一目錄 → **v1.6 徹底解決**：`_registry.json` 結構化 DB + `_index.md` 自動索引 + 篩選查詢。
3. **預分析評估的校準基數小**：已累積至 5 個案例，但仍偏少。
4. **搜尋工具依賴**：研究品質高度依賴 web_search 和 web_fetch 的可用性與回傳品質。
5. **年報 PDF 解析依賴工具鏈**：需要 pymupdf 或 poppler 才能讀取 PDF，部分環境可能未安裝。
6. **報告 metadata 格式不一致**：v1.0–v1.5 產出的報告使用 3 種不同的 metadata 格式（table / bullet / inline），sync 腳本已兼容處理，但未來新報告應統一為 bullet list 格式。

### 可能的改善方向

- ~~加入 `output/INDEX.md` 自動索引機制~~ → v1.6 已實作為 `_registry.json` + `_index.md`
- 為 Skill → Agent 的參數交接定義 schema 驗證
- 累積更多 case 以強化預分析評估的校準精度
- 探索 sub-agent 架構（Haiku 搜尋 + Opus/Sonnet 整合）降低成本
- **統一報告 metadata 格式**：建議新報告一律用 bullet list（`- **欄位**：值`），sync 腳本已優先支援
- **Registry 自動 sync hook**：在 `/company` 和 `/recon` 流程的 Step 6 存檔後自動觸發 sync（目前需手動）
- **Playwright MCP Server**（v1.8 planned，[RFC-004](rfcs/RFC-004-playwright-mcp-server.md)）：用 Playwright headless browser 解決 JS 渲染站黑名單（twincn、104、findbiz），將 entity verification 搜尋量降低 40-60%
- **多公司比較功能**：Registry 提供了結構化資料基礎，可延伸 `show` 子命令支援比較表產出
