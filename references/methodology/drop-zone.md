...
> Agent 處理 `input/{target}/` 目錄下使用者提供檔案的規範。
...
2. Step 4.0.5 直接報告「於 drop zone 找到年報：`input/{target}/xxx.pdf`，將使用」
...
- **不寫回**：agent 不應修改、覆蓋、刪除 `input/{target}/` 內任何使用者檔案
